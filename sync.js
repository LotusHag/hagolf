...
const TABLES = {
  ...
  leagues: {
    collect: keys => S.state.leagues.filter(g => keys.has(g.id)).map(g => ({ id: g.id, name: g.name, best_n: g.bestN || 0, created_by: g.createdBy || null,
      created: g.created || null, formats: S.cleanFormats(g.formats), ...S.slotOpts(g), deleted: !!g.deleted, updated_at: g.updated_at, device_id: dev() })),
    apply: r => lww(S.state.leagues, g => g.id === r.id, { id: r.id, name: r.name, bestN: r.best_n || 0, createdBy: r.created_by, created: r.created, formats: S.cleanFormats(r.formats), ...S.slotOpts(r), deleted: !!r.deleted, updated_at: iso(r.updated_at), dev: r.device_id }, r),
  },
  league_rounds: {
