...
const TABLES = {
  ...
  rounds: {
    ...
    toRow: r => ({ id: r.id, name: r.name, date: r.date || null, course: r.course, default_tee: r.defaultTee, allowance: r.allowance || 100,
      status: r.status, hole: r.hole || 0, entries: r.entries, created: r.created || null, deleted: !!r.deleted, updated_at: r.updated_at }),
    fromRow: r => ({ id: r.id, name: r.name, date: r.date, course: r.course, defaultTee: r.default_tee, allowance: r.allowance || 100,
      status: r.status, hole: r.hole || 0, entries: r.entries || [], created: iso(r.created), deleted: !!r.deleted, updated_at: iso(r.updated_at) }),
  },
  leagues: {
...
/** Pulls rows changed since the last pull and merges them; the newest updated_at wins, dirty local rows are kept. */
export async function pull() {
  ...
  try {
    for (const [table, t] of Object.entries(TABLES)) {
      const rows = await rest(`${table}?select=*&updated_at=gte.${encodeURIComponent(cursor)}&order=updated_at.asc&limit=2000`);
      const dirtyIds = new Set(S.dirty()[table] || []);
      for (const row of rows) {
        const rec = t.fromRow(row);
        const k = t.key(rec);
        if (ts(rec.updated_at) > ts(maxTs)) maxTs = rec.updated_at;
        const list = t.list();
